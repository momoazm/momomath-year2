...
function gIllnessTF(rand: Rand): Question {
  const a = pick(rand, NOT_ILL)
    return tfQ('True or false: "you might be ill" tells.', `Saying "${a}" means a person is probably ill.`, false)
}
...
function gGrow(rand: Rand): Question {
  return orderQ("Put a person's life stages in order, from youngest to oldest", ['baby', 'toddler', 'child', 'adult'], { visual: { type: 'emoji-group', emojis: ['🧒'] } })
}

const S2 = [gHealthy, gIllness, gIllnessTF, gCoverings, gBabies, gTeeth, gTeethCareTF, gGrow]

const lessons = [
  makeLesson('s2l1', 'Stay Healthy', ['2Bp.01'], 'cream', 'Fuel up!', 'Eat fruit and veg, move your body, wash hands and sleep well to stay healthy.', [gHealthy, gHealthy], gHealthy),
  ...
  makeLesson('s2l3', 'Growing Up', ['2Bp.03'], 'tails', 'Growing tall!', 'Babies grow into adults — people and animals change as they get older.', [gBabies, gGrow], gGrow),
  makeLesson('s2l4', 'Body Coverings', ['2Bs.01'], 'knuckles', 'Skin deep!', 'Skins, fur, feathers and scales keep animals comfy in their homes.', [gCoverings, gHealthy], gCoverings),
...
const boss = makeLesson('s2boss', 'Health Boss', ['2Bp.01-04', '2Bs.01-02'], 'eggman', 'BOSS TIME!', 'Eggman made a sickness ray! Answer healthy habits, growing-up and teeth to beat it.', [gHealthy, gIllness, gGrow, gTeeth], gTeeth)
