...
const HABITAT_LABEL: Record<string, string> = {
  // habitat emoji -> human-readable label
  '🌵': 'desert', '☀️': 'warm grasslands', '🌴': 'tropical jungle', '🌲': 'forest',
  '🌾': 'grassland', '🌳': 'savannah', '🕳️': 'cave', '🌊': 'ocean',
  '⛰️': 'mountain', '❄️': 'icy polar', '🦆': 'pond', '🏖️': 'coast',
  '🌸': 'meadow', '🌻': 'flowery field', '🕸️': 'web / corner', 'soil': 'soil',
  pond: 'pond',
}
const LIVING_NEED: Record<string, string[]> = {
  fish: ['water', 'food', 'air'], tree: ['water', 'air', 'sunlight'],
  dog: ['water', 'food', 'shelter'], bee: ['water', 'food', 'shelter'],
}
...
function gWhichClass(rand: Rand): Question {
  const a = pick(rand, Object.keys(LIVING_BANK))
  return mcqE(rand, `Is the ${a} a mammal, bird, fish, reptile / amphibian, or insect?`,
    pick(rand, ['mammal', 'bird', 'fish', 'reptile / amphibian', 'insect', 'human']),
    ['computer', 'cloud', 'umbrella'],
    { visual: { type: 'emoji-group', emojis: [LIVING_BANK[a as keyof typeof LIVING_BANK]] } },
  )
}
...
const S3 = [gHabitat, gLocalCompare, gNeeds, gNeedsTF, gPlantsNeed, gAdaptation, gCycle, gWhichClass, gHabitatIsHome]
...
const lessons = [
  ...
  makeLesson('s3l5', 'Match the Animal', ['2Be.01', '2Be.02'], 'knuckles', 'Right place!', 'Each animal has a body that fits where it lives — fur in the cold, webbed feet in water!', [gAdaptation, gCycle, gWhichClass], gAdaptation),
]

const boss = makeLesson('s3boss', 'Habitats Boss', ['2Be.01-03'], 'eggman', 'BOSS TIME!', 'Eggman moved all the animals to the wrong places! Match each animal to its home to fix it.', [gHabitat, gAdaptation, gCycle, gWhichClass], gHabitat)
