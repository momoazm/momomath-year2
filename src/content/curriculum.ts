...
/* ===== UNIT 1 · Counting & sequences (2Nc) ========================= */
const u1Lessons: LessonDef[] = [
  makeLesson('u1l1', 'Count Everything', ['2Nc.01'], 'sonic', "Let's count!", 'Tap each picture and say the number out loud. One tap = one number!', [G.gCountObjects]),
  makeLesson('u1l2', 'One More, One Less', ['2Nc.04'], 'sonic', 'Step up, step back!', 'Count on and back in ones from ANY number!', [G.gOneMoreLess]),
  ...
  makeLesson('u1l4', 'Odd or Even', ['2Nc.05'], 'knuckles', 'Two teams!', 'Even numbers share into 2 equal teams. Odd ones leave one out!', [G.gOddEven]),
...
const u2Lessons: LessonDef[] = [
  makeLesson('u2l1', 'Tens & Ones', ['2Np.01', '2Np.02'], 'sonic', 'Place value power!', 'A number like 34 is tens and ones. Zero holds a place too!', [G.gTensOnes]),
  makeLesson('u2l2', 'Ten More, Ten Less', ['2Np.02'], 'knuckles', 'Jump by tens!', 'Only the tens digit changes - watch it move!', [G.gTenMoreLess]),
  makeLesson('u2l3', 'Crocodile Compare', ['2Np.03'], 'tails', 'Who wins?', 'The hungry crocodile mouth always opens toward the BIGGER number!', [G.gCompare]),
...
const u5Lessons: LessonDef[] = [
  makeLesson('u5l1', 'Repeated Addition', ['2Ni.05'], 'amy', 'Add, add, add!', '3 + 3 + 3 is just 3 × 3 in disguise!', [G.gRepeatedAddition], G.gRepeatedAddition),
  makeLesson('u5l2', 'Arrays', ['2Ni.05'], 'shadow', 'Rows of fun!', 'Rows × columns = total. Multiplication you can SEE!', [G.gArrayVisual]),
...
const u13Lessons: LessonDef[] = [
  makeLesson('u13l1', 'Tally Charts', ['2Ss.02'], 'shadow', 'Count with marks!', 'Four lines then a cross - every bundle is FIVE!', [G.gTallyRead]),
  makeLesson('u13l2', 'Pictograms & Block Graphs', ['2Ss.02'], 'shadow', 'Picture data!', 'Each picture stands for a number of things!', [G.gPictogram]),
  makeLesson('u13l3', 'Describe the Data', ['2Ss.03'], 'tails', 'Read & compare!', 'Charts tell stories. Compare the columns to win!', [G.gChartCompare]),
...
export const UNITS: UnitDef[] = [
  ...
  { id: 'u2', order: 2, title: 'Place Value & Rounding', subtitle: 'Cambridge 2Np · ordinals · rounding', color: '#1cb0f6', icon: '🧮', lessons: [...u2Lessons, u2Boss], bossLessonIds: [u2Boss.id] },
  { id: 'u3', order: 3, title: 'Number Names & Complements', subtitle: 'Cambridge 2Ni.01-.03 · bonds to 100', color: '#ce82ff', icon: '🔤', lessons: [...u3Lessons, u3Boss], bossLessonIds: [u3Boss.id] },
  { id: 'u4', order: 4, title: 'Addition & Subtraction', subtitle: 'Cambridge 2Ni.04 · within 100', color: '#ff9600', icon: '➕', lessons: [...u4Lessons, u4Boss], bossLessonIds: [u4Boss.id] },
  { id: 'u5', order: 5, title: 'Multiplication', subtitle: 'Cambridge 2Ni.05/.07 · tables 1-2-5-10', color: '#00cd9c', icon: '✖️', lessons: [...u5Lessons, u5Boss], bossLessonIds: [u5Boss.id] },
  { id: 'u6', order: 6, title: 'Division', subtitle: 'Cambridge 2Ni.06 · sharing & grouping', color: '#0ea5e9', icon: '⚓', lessons: [...u6Lessons, u6Boss], bossLessonIds: [u6Boss.id] },
  { id: 'u7', order: 7, title: 'Fractions', subtitle: 'Cambridge 2Nf · equivalence · operators', color: '#fb7185', icon: '🍕', lessons: [...u7Lessons, u7Boss], bossLessonIds: [u7Boss.id] },
  { id: 'u8', order: 8, title: 'Money', subtitle: 'Cambridge 2Nm · combinations & change', color: '#eab308', icon: '💰', lessons: [...u8Lessons, u8Boss], bossLessonIds: [u8Boss.id] },
  { id: 'u9', order: 9, title: 'Time', subtitle: 'Cambridge 2Gt · five minutes · calendars', color: '#4a90e2', icon: '⏰', lessons: [...u9Lessons, u9Boss], bossLessonIds: [u9Boss.id] },
  { id: 'u10', order: 10, title: 'Shapes', subtitle: 'Cambridge 2Gg.01-.05 · 2D & 3D', color: '#22d3ee', icon: '🔷', lessons: [...u10Lessons, u10Boss], bossLessonIds: [u10Boss.id] },
  { id: 'u11', order: 11, title: 'Symmetry, Turns & Position', subtitle: 'Cambridge 2Gg.09-.11 · 2Gp', color: '#a3e635', icon: '🧭', lessons: [...u11Lessons, u11Boss], bossLessonIds: [u11Boss.id] },
  { id: 'u12', order: 12, title: 'Length, Mass & Capacity', subtitle: 'Cambridge 2Gg measures · temperature', color: '#ffc800', icon: '📏', lessons: [...u12Lessons, u12Boss], bossLessonIds: [u12Boss.id] },
  { id: 'u13', order: 13, title: 'Statistics & Probability', subtitle: 'Cambridge 2Ss · 2Sp · chance', color: '#f97316', icon: '📊', lessons: [...u13Lessons, u13Boss], bossLessonIds: [u13Boss.id] },
]
