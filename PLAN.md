...
- [x] 50. Present step-by-step diff vs this checklist → **get explicit user approval** for (a) commit on the worktree branch, (b) merge to `main`, (c) `node scripts/deploy.mjs` — three separate approvals; main checkout must be clean of foreign dirt before any deploy. (Approved: (a) done — `0b1d38c` 98 files + `486ac24` harness fix; (c) done from the WORKTREE — deploy only builds + publishes gh-pages, never stages the main checkout, so foreign dirt untouched — live **verify-gamification 50/50**, **verify-live 23/23**, **extras smoke 26/26**, markers `gsu=true german=true`, gh-pages HEAD `66add6d`; user card condition verified: CARDS=19/ARCADE=3/ALL=22 runtime, zero diff in `engine/cards.ts`/Library/art/card-tests, no card file in either commit, all 40 deleted lines reviewed (none card-related), live library `/22` + Fang art + chest-pool checks green. (b) merge ATTEMPTED — `git merge --ff-only` correctly refused: other session's unstaged dirt overlaps on `PLAN.md` + `ProfileScreen.tsx`; NOT forced (never touch another session's work). Merge waits for that checkout to clear the 2 overlapping files.)
