...
import { Q_PER_LESSON } from '../../src/content/arabic/helpers'

/** Structural validation for any question the Arabic extra can emit.
 *  Mirrors tests/german/harness.ts, but letter-tiles allow Arabic letters. */
export function expectValidArabicQuestion(q: Question) {
...
/** Run the full generic check-suite over one Arabic unit. */
export function describeUnit(unit: UnitDef) {
  describe(`arabic unit ${unit.id} · ${unit.title}`, () => {
