...
describe('shop boost purchases actually activate', () => {
  it('double-xp purchase adds a lesson charge and deducts gems', () => {
    const r = usePlayer.getState().buyItem('double-xp')
    expect(r.success).toBe(true)
    expect(usePlayer.getState().doubleXpLessons).toBe(1)
    expect(usePlayer.getState().gems).toBe(900)
  })

  it('chest-boost purchase switches the flag on; rebuy while active is rejected', () => {
    ...
    expect(usePlayer.getState().gems).toBe(gemsAfterFirst)
  })

  it('mega-chest purchase switches the flag on; rebuy while active is rejected', () => {
...
describe('boost consumption in lessons', () => {
  it('double-xp charge is consumed via useDoubleXp (the ×2 math lives in LessonScreen, proven E2E)', () => {
    ...
    expect(usePlayer.getState().doubleXpLessons).toBe(0)
  })
