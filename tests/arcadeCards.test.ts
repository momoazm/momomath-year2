...
import { CARDS, ARCADE_CARDS, ALL_CARDS, rollChest, LOCKED_PITY, STAR_THRESHOLDS } from '../src/engine/cards'
...
describe('arcade exclusives never enter the chest pool', () => {
  it('uniform chest rolls only ever pick from the 19 CARDS', () => {
    for (let i = 0; i < 20000; i++) {
      ...
      expect(CARDS.some((c) => c.id === r.cardId)).toBe(true)
    }
  })
  it('locked-pity drought rolls never force an arcade exclusive', () => {
    ...
    for (let i = 0; i < 5000; i++) {
      const r = rollChest(rng(400000 + i), 'normal', counts, LOCKED_PITY)
      expect(ARCADE_IDS.has(r.cardId)).toBe(false)
    }
  })
