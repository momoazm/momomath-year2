...
describe('leagues', () => {
  ...
  it('weekly outcome follows rank: huge XP promotes, zero XP demotes', () => {
    ...
    expect(leagueOutcomeVsBots('Bronze', 100000, '2026-08-17')).toBe('promoted')
    expect(leagueOutcomeVsBots('Bronze', 0, '2026-08-17')).toBe('demoted')
    expect(leagueOutcomeVsBots('Diamond', 100000, '2026-08-17')).toBe('promoted')
    // Mid-pack XP stays: goal-level effort lands mid-board, not auto-promoted.
    const mid = weeklyGoal('Gold')
    expect(['promoted', 'stayed', 'demoted']).toContain(leagueOutcomeVsBots('Gold', mid, '2026-08-17'))
  })
  ...
  it('bots are competitive: drives descend and goal-level effort never auto-promotes', () => {
    ...
    for (const league of ['Bronze', 'Gold', 'Diamond'] as const) {
      const outcome = leagueOutcomeVsBots(league, weeklyGoal(league), '2026-08-17')
      expect(outcome, `${league} goal-level effort`).not.toBe('promoted')
    }
    // Token effort still demotes.
    expect(leagueOutcomeVsBots('Bronze', 5, '2026-08-17')).toBe('demoted')
  })
})
