...
/** Structural validation for any question the German extra can emit.
 *  Mirrors tests/english/harness.ts, but letter-tiles allow ä ö ü ß. */
export function expectValidGermanQuestion(q: Question) {
  ...
  switch (q.kind) {
    ...
    case 'letter-tiles': {
      expect(q.targetWord).toMatch(/^[a-zäöüß]{3,14}$/)
      break
    }
    case 'truefalse': {
...
function checkLesson(id: string, lesson: LessonDef) {
  describe(`german lesson ${id}`, () => {
    it('generates exactly Q_PER_LESSON valid questions', () => {
      ...
      for (const q of qs) expectValidGermanQuestion(q)
    })
...
/** Run the full generic check-suite over one German unit. */
export function describeUnit(unit: UnitDef) {
  describe(`german unit ${unit.id} · ${unit.title}`, () => {
