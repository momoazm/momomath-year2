...
import { Q_PER_LESSON } from '../../src/content/social/helpers'

/** Structural validation for any question the social extra can emit.
 *  Mirrors tests/arabic/harness.ts. */
export function expectValidSocialQuestion(q: Question) {
...
function checkLesson(id: string, lesson: LessonDef) {
  describe(`social lesson ${id}`, () => {
    it('generates exactly Q_PER_LESSON valid questions', () => {
      ...
      for (const q of qs) expectValidSocialQuestion(q)
    })
...
/** Run the full generic check-suite over one social unit. */
export function describeUnit(unit: UnitDef) {
  describe(`social unit ${unit.id} · ${unit.title}`, () => {
