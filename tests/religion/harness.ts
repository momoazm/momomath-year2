...
import { Q_PER_LESSON } from '../../src/content/religion/helpers'

/** Structural validation for any question the religion extra can emit.
 *  Mirrors tests/arabic/harness.ts. */
export function expectValidReligionQuestion(q: Question) {
  ...
  switch (q.kind) {
    ...
    case 'letter-tiles': {
      expect(q.targetWord).toMatch(/^[\u0621-\u064A]{2,14}$/)
      break
    }
    case 'truefalse': {
...
function checkLesson(id: string, lesson: LessonDef) {
  describe(`religion lesson ${id}`, () => {
    it('generates exactly Q_PER_LESSON valid questions', () => {
      ...
      for (const q of qs) expectValidReligionQuestion(q)
    })
...
/** Run the full generic check-suite over one religion unit. */
export function describeUnit(unit: UnitDef) {
  describe(`religion unit ${unit.id} · ${unit.title}`, () => {
